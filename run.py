...
class StableRemix(StableUnCLIPImg2ImgPipeline):
    ...
    def prepare_latents(self, noise, image, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None):
...
def run_remixing(pipe, content_img, style_img, alpha, noise, **kwargs):

    content_emb = encode_image(content_img, pipe)
    style_emb = encode_image(style_img, pipe)
    
    emb = slerp(alpha, content_emb, style_emb)
    image = pipe(image=content_img, image_embeds=emb, noise=noise, **kwargs).images[0]
...
def parse_args():
    ...
    parser.add_argument('save', type=str, default=Path('remix.png'),
                        help='path to save remix image')
...
def main():
    ...
    image = run_remixing(pipe, content_img, style_img, 0.55, noise)
    print('Saving remix to', args.save)
